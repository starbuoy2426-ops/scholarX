...
MODEL_FILE = os.path.join(SCRIPT_DIR, "retriever.pkl")
...
def load_documents(folder: str) -> Tuple[List[str], List[str]]:
    ...
    if os.path.isdir(folder_path):
        ...
        candidates = sorted(os.listdir(base_folder))
    else:
        base_folder = SCRIPT_DIR
        print(f"Warning: Folder not found: {folder_path}. Falling back to text files in script directory.")
        candidates = sorted(
